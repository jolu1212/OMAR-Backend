...
def apply_rate_limit(session_data: Dict) -> bool:
    """Aplica rate limiting configurable"""
    now = datetime.now()
    rate_limit_seconds = int(os.environ.get('SESSION_RATE_LIMIT_SECONDS', 10))  # Cambiado a 10 segundos
    if session_data['last_interaction'] and (now - session_data['last_interaction']).seconds < rate_limit_seconds:
